  ...
  getResultItems(keywords) {
    ...
    this.datas.forEach(({ title, content, url }) => {
      ...
      while (indexOfContent.length !== 0) {
        ...
        const llfindex = content.lastIndexOf('\n',position)
        const start = llfindex == -1 ? Math.max(0, position - 20) : Math.max(0, position - 20, llfindex);
        const lfindex = content.indexOf('\n',position)
        //const end = Math.min(content.length, position + 80) ;
        const end = lfindex == -1 ? Math.min(content.length, position + 80) : Math.min(content.length, position + 80, lfindex);
